$.templates({
	...
	// кнопки формы
	entity_button_save: '<button type="button" class="btn btn-primary" aria-label="{{:name}}" name="{{:id}}{{:~descriptor.local_data.eid}}" onclick="entitySave(\'{{:~descriptor.local_data.container_id}}\', null);">{{:name}}</button>\n',
	entity_button_delete: '<button type="button" class="btn" aria-label="{{:name}}" name="{{:id}}{{:~descriptor.local_data.eid}}" onclick="entityDelete(\'{{:~descriptor.local_data.container_id}}\', {{:~descriptor.local_data.eid}});">{{:name}}</button>\n',
